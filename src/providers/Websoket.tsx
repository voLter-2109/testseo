...
const WebSocketComponent: FC<PropsWithChildren> = ({ children }) => {
  ...
  const handlingTextMessage = async (request: MessageWithSocket) => {
    ...
    const lastMes: LastMessage = {
      ...
      files_list: request.message.files_list,
      from_user_role: checkUserRole,
      id: request.message.id,
      new: request.message.new,
      uid: request.message.uid,
      updated_at: request.message.updated_at,
    };
  ...
  const handlingOnlineStatus = (request: RequestObjectOnlineStatus) => {
    ...
    const { is_online, user: userDataStatus, was_online_at } = request.message;
  ...
  const handlingUpdateMessage = (request: RequestUpdateMessage) => {
    ...
    const isSender = request.message.from_user.uid === user?.uid;
    const checkUserUid = isSender
      ? request.message.to_user.uid
      : request.message.from_user.uid;
  ...
  useEffect(() => {
    ...
    if (user !== null && online && apiWS) {
      ...
      socketRef.current.onmessage = (event: MessageEvent) => {
        ...
        // обработка ошибки
        if (request.error) {
          const errorMessageMatch = request.error.match(
            /'message': ErrorDetail\(string='([^']+)'/
          );
          const errorMessage = errorMessageMatch
            ? errorMessageMatch[1]
            : request.error;

          // тут поведение при неверном токене
          ...
          // обработка сообщений связанных с блокировкой
          if (
        ...
        if (
          ...
          return handlingTextMessage(request as any as MessageWithSocket);
        }
        ...
        if (
          Object.prototype.hasOwnProperty.call(request, 'action') &&
          (request.action === NEW_STATUS_USER ||
            request.action === NEW_STATUS_USER_ALL) &&
          Object.prototype.hasOwnProperty.call(request, 'status') &&
          request.status === 'OK'
        ) {
          return handlingOnlineStatus(
            request as any as RequestObjectOnlineStatus
          );
        }
        ...
        if (
          ...
          handlingDeleteMessage(request as any as RequestDeleteMEssageWS);
        }

        // изменения статуса сообщения
        if (
          Object.prototype.hasOwnProperty.call(request, 'action') &&
          request.action === CHANGE_STATUS_READ_MESSAGE &&
          Object.prototype.hasOwnProperty.call(request, 'status') &&
          request.status === 'OK'
        ) {
          return handleChangeStatusMessage(
            request as any as RequestChangeStatusMEssage
          );
        }
        ...
        if (
          ...
          return handlingUpdateMessage(request as any as RequestUpdateMessage);
        }
        return null;
      };
    }
